	...
	setup() {
		...
		return () => {
			return <section class={`flex flex-col flex-center flex-grow ${!sm.value ? 'px-2' : (attributes.value.toc ? 'ml-64' : '')}`}>
				{attributes.value.toc ? <Toc toc={toc.value} class={`flex-grow ${!sm.value ? '' : 'fixed left-0 top-8 w-64'}`} /> : ''}
				<section class="my-16 text-center">
					<h1>{attributes.value.title || t('untitled')}</h1>
					<h4>{attributes.value.desc}</h4>
				</section>
				<section class={`flex flex-row flex-wrap flex-grow justify-center ${!sm.value ? 'w-full' : 'w-4/5'} bg-ba shadow`}>
					{h(RouterView, {
						class: `flex-grow ${!sm.value ? 'px-4 py-8' : 'p-8'}`,
